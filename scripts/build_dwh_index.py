...
def main():
    ...
    # Reuse the SAME embeddings backend the app queries with, so the index and
    # the runtime can never diverge (Gemini API when GOOGLE_API_KEY is set,
    # else local MiniLM). This is the single source of truth in src/config.py.
    sys.path.insert(0, BASE_DIR)
    from src.config import embeddings as _cfg_embeddings
    _cfg_embeddings._load()
    embeddings = _cfg_embeddings._impl
    print(f"\n[*] Embedding {len(docs)} documents with {type(embeddings).__name__} ...")
