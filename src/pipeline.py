...
from src.config import BASE_DIR, SQL_MAX_RETRIES
from src.session import _get_cache_key
...
from utils.logger import _log_query

from src.pipeline_stages import chart_edit, cache_serve, enrichment, sql_phase
    ...
    try:
        ...
        if use_cache:
            _sess._query_cache[state.cache_key] = {
                ...
                "png_path": state.png_path,          # for PDF export on a future cache hit
                "full_reco": full_reco.strip(),       # summary+recos block for the PDF
                "kpi_data": kpi_data,
            }
