            ...
            with self._lock:
                if self._impl is None:
                    ...
                    if google_key:
                        ...
                        # Imported lazily so the heavy HF stack (torch, sentence-
                        # transformers) is NOT a hard dependency: production runs
                        # Gemini and can omit those packages entirely.
                        try:
                            from langchain_huggingface import HuggingFaceEmbeddings
                        except ImportError as e:
                            raise RuntimeError(
                                "No GOOGLE_API_KEY set and the local embedding backend "
                                "is unavailable (install langchain-huggingface + "
                                "sentence-transformers, or set GOOGLE_API_KEY to use "
                                "Gemini embeddings)."
                            ) from e
