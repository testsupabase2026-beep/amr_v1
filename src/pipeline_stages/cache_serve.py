...
from utils.formatting import _df_to_html, _format_number_cols, _make_kpi_card
...
async def try_exact(state: PipelineState) -> AsyncIterator[dict]:
    ...
    # Cache hits must ALSO accumulate into the report, else a session of mostly-cached
    # queries produces a PDF with only the first (cache-miss) query.
    _sess._add_recommendation_to_memory(
        state.question,
        c.get("full_reco") or f"**Summary:**\n{c.get('summary','')}",
        c.get("png_path"),
        kpi_data=c.get("kpi_data"),
    )
...
async def try_semantic(state: PipelineState) -> AsyncIterator[dict]:
    ...
    # Accumulate this (semantic) hit into the report too — use the language-correct text.
    _full = f"**Summary:**\n{summary}"
    if reco_text:
        _full += f"\n\n**Recommendations:**\n{reco_text}"
    _sess._add_recommendation_to_memory(
        state.question, _full.strip(), c.get("png_path"), kpi_data=c.get("kpi_data"),
    )
